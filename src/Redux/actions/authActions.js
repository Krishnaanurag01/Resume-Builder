...
export const register = (userData) => {
    return (dispatch,getState,{getFirestore,getFirebase})=>{
        dispatch(registerReq()) // now loading becomes true
        ...
        firebase.auth().createUserWithEmailAndPassword(userData.email,userData.password).then(async(data)=>{
            ...
            dispatch(registerSuc()) //this will make loading false
        }).catch((err)=>{
            dispatch(registerFail(err)) // sending error and loading true
            setTimeout(()=>{
                dispatch(removeError()) // also removing error after 2 sec
            },2000)
        })
    }
}
